...
Base URL `https://enrich.moroku.com`. Auth: per-tenant bearer key (`Authorization: Bearer mk_live_…`) validated by a **Lambda authorizer on an HTTP API (v2)** — not REST-API usage plans, which HTTP APIs don't support. Keys are stored SHA-256-hashed in a `tenants` table (tenant id, name, status, plan); the authorizer resolves key → tenant context, and its result is cached ~5 minutes keyed on the token. Per-tenant rate limiting is the *intended* design (soft quota counters in the authorizer, DynamoDB-backed) but **is not yet implemented** — see the note below. OAuth client-credentials later if a customer requires it.

> **Implementation note (as-built, 15 Aug 2026 — ENR-5).** `quota` is *stored* on the tenant row (default 1,000,000) but **no code reads it**, and the authorizer performs a lookup only (no counter writes). The **only live protection today is stage-level throttling on the HTTP API `$default` stage — 200 burst / 100 req/s, shared across all tenants, not per-tenant** — so one looping consumer can exhaust the budget for everyone. Per-tenant soft-quota enforcement in the authorizer is deferred until per-tenant limits are actually needed (arrives with the second tenant sending volume). Likewise `plan` is resolved into the authorizer context but is **not consumed anywhere at request time** — the `internal`/`trial`/`commercial` gate is enforced only by the `assertPlanAllowed` check in the issuance script, not server-side. All endpoints JSON. All writes idempotent via `Idempotency-Key` header.
